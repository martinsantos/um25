...
echo "🔧 Solucionando límite de subida de nginx para las imágenes restantes..."

# Las 3 imágenes que fallaron con error 413
FAILED_IMAGES=(
...
echo "📋 Imágenes que fallaron por tamaño:"
for img in "${FAILED_IMAGES[@]}"; do
    if [ -f "imagenes_antecedentes_versionproduccion/$img" ]; then
        size=$(ls -lh "imagenes_antecedentes_versionproduccion/$img" | awk '{print $5}')
        echo "  - $img ($size)"
    fi
done
...
echo ""
echo "# 3. Agregar o modificar la línea dentro del bloque server para Directus:"
...
# Crear script específico para las imágenes restantes
cat > import-remaining-images.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x import-remaining-images.py

echo ""
echo "✅ Script creado: import-remaining-images.py"
echo "📝 Este script subirá solo las 3 imágenes que fallaron" 