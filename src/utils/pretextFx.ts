...
const H_FONT = '"Futura PT","Futura","Trebuchet MS",sans-serif';
...
/* Cursor */
.ptfx-cursor{display:inline-block;width:3px;margin-left:2px;background:var(--c,#ef4444);animation:_bk .53s steps(1) infinite;vertical-align:baseline}
...
.ptfx-stamp{position:absolute;display:inline-block;color:#dc2626;border:3px solid #dc2626;padding:4px 14px;font:900 18px/1 'Futura PT',sans-serif;text-transform:uppercase;letter-spacing:.15em;border-radius:4px;opacity:0;transform:scale(3.5) rotate(-18deg);pointer-events:none;mix-blend-mode:multiply;filter:drop-shadow(0 1px 0 rgba(0,0,0,.3))}
...
.ptfx-char.glitch::before{color:#ef4444;transform:translateX(-1px);mix-blend-mode:screen}
...
.ptfx-hud .rec{color:#ef4444;animation:_bk 1s steps(1) infinite}
...
.ptfx-alarm::before{content:'';position:absolute;inset:-8px;border:2px solid #ef4444;opacity:.1;animation:_alarm 1.2s ease-in-out infinite;pointer-events:none}
...
@keyframes _almflash{0%{color:#ef4444;text-shadow:0 0 8px #ef4444}100%{color:white;text-shadow:none}}
...
export function heroFx(selector: string | HTMLElement, text: string, opts: HeroOpts = {}) {
  ...
  if (!el || !text) return;
  const { lines = 3, color = '#ef4444', speed = 18, weight = 700, style = 'typewriter', onDone, ambient } = opts;
