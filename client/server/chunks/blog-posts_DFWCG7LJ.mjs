...
function getPostsByFilters(filters) {
  let filteredPosts = [...blogPosts];
  if (filters.category) {
    filteredPosts = filteredPosts.filter(
      (post) => post.categories?.includes(filters.category || "")
    );
  }
  if (filters.tag) {
    filteredPosts = filteredPosts.filter(
  ...
  if (filters.searchQuery) {
    ...
    filteredPosts = filteredPosts.filter(
      (post) => post.title.toLowerCase().includes(searchLower) || post.excerpt.toLowerCase().includes(searchLower) || post.content.toLowerCase().includes(searchLower)
    );
  }
...
export { getCategories as a, getPostsByFilters as b, getAllPosts as g };
