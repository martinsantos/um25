/* empty css                                 */
import { b as createAstro, c as createComponent, r as renderComponent, a as renderTemplate, m as maybeRenderHead, e as addAttribute } from '../chunks/astro/server_B7qweUek.mjs';
import 'kleur/colors';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  async function fetchData() {
    ...
    try {
      ...
      servicios = result.data.map((servicio) => ({
        ...servicio,
        imagen: servicio.Imagen ? `/api/asset/${servicio.Imagen}?width=400&height=250&fit=cover&quality=80` : getRandomImage(),
  ...
  await fetchData();
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Servicios" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="container mx-auto px-4 py-8"> <nav class="mb-8"><ol class="flex items-center space-x-2 text-gray-600"><li><a href="/" class="hover:text-blue-600">Inicio</a></li><li><span class="mx-2">/</span></li><li class="text-blue-600 font-medium">Servicios</li></ol></nav> <h1 class="text-4xl font-bold mb-8">Nuestros Servicios</h1> <form method="GET" action="/servicios" class="bg-white p-6 rounded-lg shadow-lg mb-8" id="filterForm"> <input type="hidden" name="page" value="1"> <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end"> ${Object.entries(FILTER_CONFIG).map(([key, config]) => renderTemplate`<div class="space-y-1"> <label${addAttribute(`filter-${key}`, "for")} class="block text-sm font-medium text-gray-700">${config.display}</label> <select${addAttribute(`filter-${key}`, "id")}${addAttribute(key, "name")} class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm"> <option value="">Todos</option> ${config.options.map((option) => renderTemplate`<option${addAttribute(option.id, "value")}${addAttribute(activeFilters[key] === option.id, "selected")}>${option.name}</option>`)} </select> </div>`)} <div class="space-y-1"> <label for="search-input" class="block text-sm font-medium text-gray-700">Buscar</label> <div class="relative"> <input type="search" id="search-input" name="q"${addAttribute(searchQuery, "value")} placeholder="Buscar por título..." class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm pr-10"> <button type="submit" class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500 hover:text-blue-600" aria-label="Buscar"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"></path></svg> </button> </div> </div> </div> <div class="mt-6 text-right"> <button type="submit" class="px-5 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
Aplicar Filtros / Buscar
</button> </div> </form> ${(Object.values(activeFilters).some(Boolean) || searchQuery) && renderTemplate`<div class="mb-6"> <div class="bg-blue-50 p-4 rounded-lg border border-blue-200"> <div class="flex items-center justify-between mb-2 flex-wrap gap-2"> <div class="flex items-center gap-2"> <svg class="w-5 h-5 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg> <span class="font-medium text-blue-700"> ${totalItems === 1 ? "1 resultado encontrado" : `${totalItems} resultados encontrados`} ${searchQuery ? ` para "${searchQuery}"` : ""} ${Object.values(activeFilters).some(Boolean) ? " con los filtros actuales" : ""} </span> </div> <a href="/servicios" class="text-sm text-red-600 hover:text-red-800 font-medium flex items-center gap-1"> Limpiar todo <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg> </a> </div> <div class="flex flex-wrap gap-2"> ${searchQuery && renderTemplate`<span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800"> Búsqueda: "${searchQuery}" <a${addAttribute(buildPageUrl(1), "href")}${addAttribute((e) => {
    e.preventDefault();
    const url = new URL(window.location.href);
    url.searchParams.delete("q");
    url.searchParams.set("page", "1");
    window.location.href = url.toString();
  }, "onclick")} class="ml-2 text-red-500 hover:text-red-700 focus:outline-none" title="Eliminar búsqueda"> × </a> </span>`} ${Object.entries(activeFilters).map(([key, value]) => value && renderTemplate`<span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800"> ${FILTER_CONFIG[key].display}: ${value} <a${addAttribute(buildPageUrl(1, key), "href")} class="ml-2 text-red-500 hover:text-red-700 focus:outline-none"${addAttribute(`Eliminar filtro ${FILTER_CONFIG[key].display}`, "title")}> × </a> </span>`)} </div> </div> </div>`} ${error && renderTemplate`<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6" role="alert"> <strong class="font-bold">Error: </strong> <span class="block sm:inline">${error}</span> </div>`} ${servicios.length > 0 && renderTemplate`<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"> ${servicios.map((servicio) => renderTemplate`<article class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow duration-300 overflow-hidden flex flex-col"> <a${addAttribute(`/servicios/${servicio.id}/${generateSlug(servicio.Titulo)}`, "href")} class="block group h-full flex flex-col"> <div class="relative h-48 w-full overflow-hidden"> <img${addAttribute(servicio.imagen, "src")}${addAttribute(servicio.titulo || "Servicio", "alt")} class="w-full h-full object-cover object-center transition-transform duration-300 group-hover:scale-105" loading="lazy" width="400" height="250"> </div> <div class="p-4 flex flex-col flex-grow"> <h2 class="text-lg font-semibold text-gray-800 mb-2 group-hover:text-blue-600 transition-colors">${servicio.titulo}</h2> <p class="text-gray-600 text-sm mb-4 line-clamp-3 flex-grow">${servicio.descripcion}</p> <div class="mt-auto border-t border-gray-100 pt-3 space-y-1 text-xs text-gray-500"> ${servicio.area && renderTemplate`<div class="flex items-center truncate"><svg class="w-3.5 h-3.5 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg><span class="truncate">${servicio.area}</span></div>`} ${servicio.cliente && renderTemplate`<div class="flex items-center truncate"><svg class="w-3.5 h-3.5 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg><span class="truncate">${servicio.cliente}</span></div>`} ${servicio.unidad_negocio && renderTemplate`<div class="flex items-center truncate"><svg class="w-3.5 h-3.5 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg><span class="truncate">${servicio.unidad_negocio}</span></div>`} </div> </div> </a> </article>`)} </div>`} ${totalPages > 1 && !error && renderTemplate`<nav class="mt-8 flex flex-col sm:flex-row justify-between items-center gap-4" aria-label="Paginación"> <div class="text-sm text-gray-700"> Mostrando <span class="font-medium">${Math.min((currentPage - 1) * PAGE_SIZE + 1, totalItems)}</span> a <span class="font-medium">${Math.min(currentPage * PAGE_SIZE, totalItems)}</span> de <span class="font-medium">${totalItems}</span> resultados </div> <ul class="inline-flex items-center -space-x-px text-sm"> <li> <a${addAttribute(currentPage > 1 ? buildPageUrl(currentPage - 1) : "#", "href")}${addAttribute(`flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 ${currentPage === 1 ? "opacity-50 cursor-not-allowed" : ""}`, "class")}${addAttribute(currentPage === 1, "aria-disabled")}${addAttribute(currentPage === 1 ? -1 : void 0, "tabindex")}> <span class="sr-only">Anterior</span> <svg class="w-2.5 h-2.5 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 1 1 5l4 4"></path></svg> </a> </li> ${generatePaginationNumbers(currentPage, totalPages).map((page, index) => renderTemplate`<li${addAttribute(`page-${page}-${index}`, "key")}> ${typeof page === "number" ? renderTemplate`<a${addAttribute(buildPageUrl(page), "href")}${addAttribute(`flex items-center justify-center px-3 h-8 leading-tight border border-gray-300 ${currentPage === page ? "text-blue-600 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 font-medium" : "text-gray-500 bg-white hover:bg-gray-100 hover:text-gray-700"}`, "class")}${addAttribute(currentPage === page ? "page" : void 0, "aria-current")}> ${page} </a>` : renderTemplate`<span class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300">...</span>`} </li>`)} <li> <a${addAttribute(currentPage < totalPages ? buildPageUrl(currentPage + 1) : "#", "href")}${addAttribute(`flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 ${currentPage === totalPages ? "opacity-50 cursor-not-allowed" : ""}`, "class")}${addAttribute(currentPage === totalPages, "aria-disabled")}${addAttribute(currentPage === totalPages ? -1 : void 0, "tabindex")}> <span class="sr-only">Siguiente</span> <svg class="w-2.5 h-2.5 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"></path></svg> </a> </li> </ul> </nav>`} ${servicios.length === 0 && !error && renderTemplate`<div class="text-center py-16"> <div class="text-gray-400 mb-4"> <svg class="w-16 h-16 mx-auto" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"> <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607zM11.995 12h.01"></path> </svg> </div> <h3 class="text-xl font-semibold text-gray-800 mb-2">No se encontraron servicios</h3> <p class="text-gray-500"> ${searchQuery || Object.values(activeFilters).some(Boolean) ? "Prueba ajustar los filtros o el término de búsqueda." : "No hay servicios disponibles en este momento."} </p> ${(searchQuery || Object.values(activeFilters).some(Boolean)) && renderTemplate`<a href="/servicios" class="mt-4 inline-block px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium"> Limpiar búsqueda y filtros </a>`} </div>`} </main> ` })}`;
}, "/Users/Shared/Files From d.localized/D/ultima milla/2024/MKT 2024/umw141024/umw46-main/fumbling-field/src/pages/servicios/index.astro", void 0);
const $$file = "/Users/Shared/Files From d.localized/D/ultima milla/2024/MKT 2024/umw141024/umw46-main/fumbling-field/src/pages/servicios/index.astro";
const $$url = "/servicios";
